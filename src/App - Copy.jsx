...
function PurchasesAddForm({ ctx }) {
  ...
  const [date, setDate] = useState(todayISO());
  const blankRow = () => ({ key: uid(), lotId: "", rate: "" });
  const [rows, setRows] = useState([blankRow()]);
  const [lastSaved, setLastSaved] = useState(null);
  ...
  const saveAll = () => {
    ...
    const batchId = uid();
    const newOnes = valid.map((r) => ({ id: uid(), batchId, lotId: r.lotId, weight: lotInfo[r.lotId].remaining, rate: Number(r.rate), date }));
  ...
  return (
    <div>
      ...
      <div className="ticket-form" style={{ maxWidth: 760 }}>
        <Field label="Date"><input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></Field>
        <FormDivider label="Purchase lines for this entry" />
        <div className="rows-table">
          <div className="rows-head cols-4"><span>Reel (search lot no / brand)</span><span>Weight available</span><span>Rate / kg</span><span>Amount</span></div>
          {rows.map((r) => {
            const lot = reels.find((x) => x.id === r.lotId);
            const weight = lot ? lotInfo[lot.id].remaining : 0;
            const amt = weight * Number(r.rate || 0);
            return (
              <div className="rows-line cols-4" key={r.key}>
                ...
                {b.lines.map((p) => {
                  ...
                  return (
                    <div className="row" key={p.id}>
                      ...
                      <div className="row-sub">{num(p.weight)} kg at {num(p.rate)}/kg = {money(Number(p.weight) * Number(p.rate))}</div>
                    </div>
                  );
                })}
              </div>
...
function PurchaseReportView({ ctx }) {
  ...
  const doPrint = () => {
    ...
    groups.forEach(([d, lines]) => {
      ...
      html += `<h2>${esc(fmtDate(d))}</h2><table><thead><tr><th>Entry</th><th>Description</th><th>Lot no</th><th>T.weight</th><th>Rate</th><th>Amount</th></tr></thead><tbody>`;
      sorted.forEach((p) => {
        const lot = reels.find((r) => r.id === p.lotId); if (!lot) return;
        html += `<tr><td class="tag">${esc(purchaseLabelMap.get(p.batchId))}</td><td>${esc(reelDesc(lot))}</td><td class="tag">${esc(lot.lotNo)}</td><td>${num(p.weight)}</td><td>${num(p.rate)}</td><td>${money(Number(p.weight) * Number(p.rate))}</td></tr>`;
      });
      html += `</tbody><tfoot><tr><td colspan="3">${lines.length} lot(s)</td><td>${num(tW)}</td><td></td><td>${money(tA)}</td></tr></tfoot></table>`;
    });
      ...
      {groups.map(([d, lines]) => {
        ...
        return (
          <div key={d} className="date-block">
            <div className="date-block-head">{fmtDate(d)}</div>
            <table className="ledger-table">
              <thead><tr><th>Entry</th><th>Item description</th><th>Lot no</th><th>T.weight</th><th>Rate</th><th>Amount</th></tr></thead>
              <tbody>
                {sorted.map((p) => {
                  const lot = reels.find((r) => r.id === p.lotId);
                  if (!lot) return null;
              ...
              <tfoot><tr><td /><td colSpan={2}>{lines.length} lot(s)</td><td className="mono">{num(tW)}</td><td /><td className="mono">{money(tA)}</td></tr></tfoot>
            </table>
          </div>
        );
      })}
...
function PurchaseEditTab({ ctx }) {
  ...
  const startEdit = (p) => { setEditId(p.id); setEf({ lotId: p.lotId, rate: p.rate, date: p.date }); };
  const saveEdit = () => {
    const weight = remainingForLot(ef.lotId, editId);
    persist.purchases(purchases.map((p) => p.id === editId ? { ...p, lotId: ef.lotId, weight, rate: Number(ef.rate), date: ef.date } : p));
    setEditId(null);
  };
...
function StockReportTab({ ctx }) {
  ...
  const doPrint = () => {
    let html = `<table><thead><tr><th>Description</th><th>Detail</th><th>Lot no</th><th>Width</th><th>Supplier</th><th>Status</th><th>Remaining</th><th>Weight</th></tr></thead><tbody>`;
    sorted.forEach((lot) => {
      const info = lotInfo[lot.id];
      html += `<tr><td>${esc(reelDesc(lot))}</td><td>${esc(lot.detail || "—")}</td><td class="tag">${esc(lot.lotNo)}</td><td>${esc(lot.width)}</td><td>${esc(supplierName(lot.supplierId))}</td><td>${esc(statusLabel(info.status))}</td><td>${num(info.remaining)}</td><td>${num(lot.weight)}</td></tr>`;
    });
    html += `</tbody><tfoot><tr><td colspan="6">${sorted.length} lot(s)</td><td>${num(sumRemaining)}</td><td>${num(sumWeight)}</td></tr></tfoot></table>`;
  ...
  return (
    <div>
      ...
      <table className="ledger-table">
        <thead><tr><th>Item description</th><th>Detail</th><th>Lot no</th><th>Width</th><th>Supplier</th><th>Status</th><th>Remaining</th><th>Weight</th></tr></thead>
        <tbody>
          {sorted.length === 0 && <tr><td colSpan={8}><EmptyRow>No lots match this filter.</EmptyRow></td></tr>}
          {sorted.map((lot) => {
            const info = lotInfo[lot.id];
            return (
              <tr key={lot.id}>
        ...
        {sorted.length > 0 && (
          <tfoot><tr><td colSpan={6}>{sorted.length} lot(s)</td><td className="mono">{num(sumRemaining)}</td><td className="mono">{num(sumWeight)}</td></tr></tfoot>
        )}
      </table>
    </div>
  );
  ...
  return (
    <style>{`
      ...
      @media (max-width:640px){
        .rows-head, .rows-line, .rows-head.cols-3, .rows-line.cols-3, .rows-head.cols-4, .rows-line.cols-4, .rows-head.cols-7, .rows-line.cols-7 { grid-template-columns:1fr; }
